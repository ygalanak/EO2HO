...
#Barplot for effect.
plot_ly(data=period2_NUTS1) %>% add_bars(x=~Name, y=~Diff, showlegend=FALSE,
...
p <- ggplot(data=shp1, aes(x=long, y=lat, group=group, fill=Diff, alpha=Significant)) + geom_polygon() + coord_equal() + theme_void() + labs(title="Spillover effect between announcement and start of EO2HO", subtitle="Difference in ratio of registrations (2020/2019) of EO2HO postcodes vs All postcodes", fill="Difference in ratio of registrations")
p + scale_fill_gradient2(low="#a2c5f5", mid="#dff2cb", high="#f7b231", midpoint=0) + scale_alpha_manual(values=c(0.3, 1))
...
#Map of restaurants.
map_rest <- ggplot(data=shp2, aes(x=long, y=lat, group=group, fill=n)) + geom_polygon() + coord_equal() + 
  theme_void() + labs(title="Participating restaurants in EO2HO", subtitle=" ", fill="Number") + scale_fill_gradient(low="#dff2cb", high="#f7b231")
map_rest
...
#NUTS1 level.
r_spillover_lm <- lm(arrange(period2_NUTS1, period2_NUTS1$NUTS1)$All_Postcodes ~ r_region$n[c(1:12)])
summary(r_spillover_lm)

plot_ly(x=r_region$n[c(1:12)], y=arrange(period2_NUTS1, period2_NUTS1$NUTS1)$All_Postcodes, text=nuts1_names$Name, type='scatter', mode='text+markers', textposition="middle right", name="") %>% 
  add_trace(y=(r_region$n[c(1:12)]*r_spillover_lm$coefficients[2]+r_spillover_lm$coefficients[1]), type='scatter', mode='lines', showlegend=FALSE, name="") %>% 
...
plot_ly(data=period2_Area, x=~R, y=~Ratio, type='scatter', mode='markers', text=~PostcodeArea) %>%
  layout(title=list(text="EO2HO restaurants vs Ratio of registrations in Other sectors"),
         yaxis=list(title="Ratio of registrations (2020/2019)"), 
         xaxis=list(title="Number of EO2HO restaurants"))
